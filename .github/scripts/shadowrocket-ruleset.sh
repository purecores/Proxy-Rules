...
URL_TELEGRAMIP="https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/meta/geo/geoip/telegram.yaml"
URL_GOOGLEIP="https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/meta/geo/geoip/google.yaml"
URL_CNIP="https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/meta/geo/geoip/cn.yaml"

# 输出（Shadowrocket geoip list）
OUT_GEO_TELEGRAMIP="compilation/shadowrocket/geoip-telegram.list"
OUT_GEO_GOOGLEIP="compilation/shadowrocket/geoip-google.list"
OUT_GEO_CNIP="compilation/shadowrocket/geoip-cn.list"

# 输入（远程 mihomo geosite domain_suffix rule-set）,输出（Shadowrocket geosite list）
URL_TELEGRAM="https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/meta/geo/geosite/telegram.yaml"
OUT_GEO_TELEGRAM="compilation/shadowrocket/geosite-telegram.list"
...
# 下载远程 yaml 后解析为 DOMAIN-SUFFIX（Shadowrocket 格式）
# 适用于 geosite 中的 domain_suffix 列表
convert_geosite_yaml_url_to_shadowrocket_list() {
  local url="$1"
  local out_file="$2"
  local tmp_yaml
  tmp_yaml="$(mktemp)"
...
for item in payload:
    ...
    # - "DOMAIN-SUFFIX,example.com"       => 原样
    # - "domain_suffix:example.com"       => DOMAIN-SUFFIX,example.com
    if ":" in s and s.split(":", 1)[0].strip().lower() in ("domain_suffix", "suffix"):
        s = s.split(":", 1)[1].strip()

    if s.upper().startswith("DOMAIN-SUFFIX,"):
        rule = s
    else:
        if s.startswith("+."):
            s = s[2:].strip()
...
header = [
    "# Generated by GitHub Actions",
    f"# Source: {src_url}",
    "# Source: mihomo rule-set (geosite/domain_suffix) yaml -> Shadowrocket .list",
]
outp.write_text("\n".join(header + lines) + "\n", encoding="utf-8")
PY
}

echo "[1/3] Convert local domain rule-sets..."
...
echo "[2/3] Download & convert remote geoip (ip) rule-sets..."
convert_geoip_yaml_url_to_shadowrocket_list "$URL_TELEGRAMIP" "$OUT_GEO_TELEGRAMIP"
convert_geoip_yaml_url_to_shadowrocket_list "$URL_GOOGLEIP"   "$OUT_GEO_GOOGLEIP"
convert_geoip_yaml_url_to_shadowrocket_list "$URL_CNIP"       "$OUT_GEO_CNIP"

echo "[3/3] Download & convert remote geosite (domain_suffix) rule-sets..."
convert_geosite_yaml_url_to_shadowrocket_list "$URL_TELEGRAM" "$OUT_GEO_TELEGRAM"
